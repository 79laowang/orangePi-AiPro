...
| NPU 利用率 | AICore 0-78% |
| 内存占用 | 约 2-3 GB |
| 生成 100 tokens | 约 80 秒 |

### 性能对比分析

| 硬件 | 推理框架 | tokens/s | 备注 |
|-----|---------|----------|------|
| NVIDIA H20 | Transformers | 58 | 官方基准 |
| NVIDIA H20 | SGLang | 414 | 专用推理引擎 |
| Ascend 910B | CloudMatrix-Infer | 538 | 华为官方 |
| **Ascend 310B4** | **mindnlp** | **~1.2** | 本项目实测 |

**性能差距原因：**
- **硬件限制**: Ascend 310B4 (8-12 TOPS) << NVIDIA H20 (~148 TFLOPS)
- **软件栈**: mindnlp 是通用框架，非专用推理引擎
- **优化程度**: 未使用 Flash Attention、KV Cache 优化等
